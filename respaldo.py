...
async def manejar_nivel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja el nivel de reporte y pide el tipo de reporte."""
    query = update.callback_query
    await query.answer()
    ...
    # Para la alerta roja, ahora preguntamos si es operacional o comunicacional
    if nivel == "roja":
        pregunta = "Seleccione el tipo de reporte para la alerta roja:"
        ...
        # Para alertas verdes, saltamos las preguntas de violencia y amenaza
        context.user_data["violencia"] = "no"
        context.user_data["amenaza_vida"] = "no"
        pregunta = "¿Se realizó una verificación del evento?"
        keyboard = [[InlineKeyboardButton("Sí", callback_data="si"), InlineKeyboardButton("No", callback_data="no")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(pregunta, reply_markup=reply_markup)
        return ESTADO_VERIFICADO
    else: # Esto aplica para los niveles "amarilla" y "naranja"
        pregunta = "Seleccione el tipo de reporte:"
        keyboard = [[InlineKeyboardButton("Operacional", callback_data="operacional"), InlineKeyboardButton("Comunicacional", callback_data="comunicacional")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(pregunta, reply_markup=reply_markup)
        return ESTADO_TIPO_REPORTE
...
async def manejar_tipo_reporte(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Dirige el flujo según el tipo de reporte seleccionado."""
    query = update.callback_query
    await query.answer()
    context.user_data["tipo_reporte"] = query.data
    
    # Lógica específica para Alerta Roja: se pide el tipo de evento como texto libre.
    if context.user_data["nivel"] == "Roja":
        pregunta = "Por favor, ingresa el tipo de evento:"
        await query.edit_message_text(pregunta)
        return ESTADO_TIPO_EVENTO_TEXTO_ROJA

    # Lógica para otros niveles (Amarilla, Naranja)
    if query.data == "operacional":
        pregunta = "Por favor, ingresa el tipo de evento:"
        await query.edit_message_text(pregunta)
        return ESTADO_TIPO_EVENTO
    else: # Flujo comunicacional (Amarilla, Naranja)
        pregunta = "Por favor, ingresa el tipo de evento:"
        await query.edit_message_text(pregunta)
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            ESTADO_TIPO_EVENTO: [MessageHandler(filters.TEXT & ~filters.COMMAND, manejar_tipo_evento)],
            ...
            ESTADO_TIPO_EVENTO_COMUNICACIONAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, manejar_tipo_evento_comunicacional)],
            ...
            ESTADO_TIPO_EVENTO_TEXTO_ROJA: [MessageHandler(filters.TEXT & ~filters.COMMAND, manejar_tipo_evento_texto_roja)],
            ESTADO_TIPO_EVENTO_ROJA: [MessageHandler(filters.TEXT & ~filters.COMMAND, manejar_tipo_evento_roja)],
