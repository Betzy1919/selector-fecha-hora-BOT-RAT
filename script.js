...
const seleccion = {
  ...
  mes: "",
  anio: "",
  hora: "", // Se almacena en 24h internamente (00-23)// script.js (VERSIÓN FINAL CON SELECTOR DE HORA EN 12H, CENTRADO Y RANGO DE AÑOS)
...
function confirmar() {
  // Esta función ahora solo es llamada por el Main Button listener
  ...
  if (window.Telegram && window.Telegram.WebApp && Telegram.WebApp.sendData) {
      // Enviar los datos. Ya no necesitamos setTimeout ni Telegram.WebApp.close(),
      // ya que el Main Button lo hace de forma más confiable.
...
function inicializar() {
  // ... (todo tu código actual de inicialización: generar listas, listeners, etc.) ...
  
  // ----------------------------------------------------------------
  // CLAVE: Lógica del Botón Principal de Telegram (MainButton)
  // ----------------------------------------------------------------
  if (window.Telegram && window.Telegram.WebApp) {
      const MainButton = Telegram.WebApp.MainButton;
      
      // 1. Configurar el botón
      MainButton.setText("✅ Confirmar Fecha y Hora");
      MainButton.show(); // Mostrar el botón

      // 2. Asignar el listener de clic a la función confirmar()
      MainButton.onClick(confirmar);

      // 3. Asegurar la inicialización total (si no lo tienes en index.html)
      Telegram.WebApp.ready();
  }
  // ----------------------------------------------------------------
}

// Asegúrate de que tu document.addEventListener("DOMContentLoaded", inicializar)
// permanezca al final de script.js para que el botón se configure al cargar.
