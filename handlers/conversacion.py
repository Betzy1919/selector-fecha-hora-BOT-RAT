...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 2. Reinicia los datos de la sesión anterior
    # Si esta línea se ejecuta, BORRA cualquier dato guardado anteriormente.
    context.user_data.clear()

# 3. Vuelve a guardar el nombre de usuario, asegurando que esté disponible para la nueva sesión
    # ❌ ESTA LÍNEA ES CRÍTICA Y DEBE EJECUTARSE DESPUÉS DEL CLEAR
    context.user_data["nombre_usuario"] = user_name

    welcome_message = "Bienvenido al sistema automatizado de Reportes de alertas tempranas de FONPESCA. Este canal es exclusivo para personal autorizado.\n\nPor favor, ingresa su número de cédula para continuar."
    if update.message:
...
async def manejar_cedula(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not cedula_input.isdigit():
        await update.message.reply_text("❌ ¡Cédula inválida, ingresa solo números!\n\nPor favor, ingresa nuevamente su número de cédula para continuar. ")
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if result:
                    ...
                    await update.message.reply_text("❌ Acceso denegado. La cédula ingresada no está autorizada. \n\n Presione cualquier tecla o número para iniciar de nuevo.")
...
async def reintentar_cedula(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text("Ingrese nuevamente la cédula ")
...
# Mostrar resumen inicial solo una vez
 ...
 #  Guarda el estado actual en el historial antes de avanzar
   ...
   # El teclado de Tipo de Reporte
    ...
    # 2. Fusionar el teclado con los botones de navegación
    reply_markup = InlineKeyboardMarkup(merge_keyboard_with_navigation(keyboard)) # <-- MODIFICADO
    
    # ✅ Eliminar el mensaje de selección de nivel
    try:
        if query.message:
            await context.bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)
    except Exception as e:
        print(f"⚠️ No se pudo borrar el mensaje de nivel: {e}")
...
# 4. guardar_en_historial
    ...
    # 1. BIFURCACIÓN CLAVE SEGÚN EL TIPO DE REPORTE
    if tipo == "comunicacional":
        ...
        "📌 Por favor, selecciona el **tipo de evento comunicacional** que deseas reportar:",
        ...
        # ✅ CORRECCIÓN SINTÁCTICA: Se eliminan los paréntesis ()
        if nivel == "verde":
        ...
        mensaje_pregunta = await query.message.reply_text(
            "📌 Por favor, selecciona el **tipo de evento operacional** que deseas reportar:",
            reply_markup=InlineKeyboardMarkup(merge_keyboard_with_navigation(opciones)),
            parse_mode='Markdown'
        )
        context.user_data["pregunta_id"] = mensaje_pregunta.message_id
...
async def manejar_tipo_evento_comunicacional(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 🔍 Detectar si viene de modificación (SETUP)
    ...
    # 📋 Actualizar resumen y limpiar pregunta anterior (necesario para ambos flujos)
    await actualizar_resumen_estatico(update, context)
    await borrar_pregunta_anterior(context, update)
    
    if context.user_data.get("en_modo_modificacion"):
            
            # 🗑️ CORRECCIÓN CRÍTICA: Eliminar el mensaje actual del usuario (SOLUCIÓN DE LA PREGUNTA PERSISTENTE)
            if update.message:
                try:
            ...
            # ✅ Flujo de MODIFICACIÓN: Llama al post-manejador para limpiar banderas y pedir la confirmación.
            return await manejar_post_modificacion(update, context)
        
    else:
            # ➡️ Flujo NORMAL:
            
            # 5a. Guardar en historial (para el botón 'Volver')
            guardar_en_historial(context, ESTADO_TIPO_EVENTO_COMUNICACIONAL)
        
        # 2. 📝 Pregunta siguiente: descripción del evento
...
async def manejar_tipo_evento_texto_roja(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 🔐 1. Detectar y configurar modo modificación si aplica (SETUP)
    if context.user_data.get("pregunta_modificacion_id"):
        context.user_data["en_modo_modificacion"] = True
        context.user_data.pop("tipo_evento", None)  # Eliminar respuesta anterior
        context.user_data['campo_a_modificar'] = 'tipo_evento' 

    # 📝 2. Capturar texto (Aplica a ambos flujos)
    texto = update.message.text.strip()
    context.user_data["tipo_evento"] = texto

    # ✅ 3. Verificar modificación desde resumen (INTERRUPCIÓN por navegación)
    modificacion = await verificar_modificacion(update, context)
    if modificacion:
        # Se elimina el estado del historial si es una navegación que interrumpe el flujo de entrada
        eliminar_estado_del_historial(context, ESTADO_TIPO_EVENTO_TEXTO_ROJA)
        return modificacion

    # 📋 4. Actualizar resumen y limpiar pregunta anterior (Aplica a ambos flujos)
    await actualizar_resumen_estatico(update, context)
    await borrar_pregunta_anterior(context, update)

    # 🔑 5. Bifurcación CLAVE: Flujo de Modificación vs. Flujo Normal
    if context.user_data.get("en_modo_modificacion"):
        # ✅ Flujo de MODIFICACIÓN:
        # Se capturó el dato. Se llama al post-manejador para limpiar banderas 
        # y pedir la confirmación de la modificación.
        return await manejar_post_modificacion(update, context)
        ...
        # 5b. 🧾 Asignar valores predeterminados para alerta roja (para saltar pasos)
        context.user_data["descripcion_evento"] = "Alerta de máxima alerta"
        ...
        # 5c. 📌 Pregunta siguiente: violencia
...
async def manejar_descripcion_comunicacional(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja la descripción del evento comunicacional y pasa a la pregunta de Tipo de Medio (botones)."""
    
    # 📝 1. Capturar texto y guardar dato
    texto = update.message.text.strip()
    context.user_data["descripcion"] = texto
    
    # 🔐 2. Flujo de Modificación (Mantiene la lógica de modificación)
    ...
    # ➡️ 3. Flujo NORMAL (FIX: Envía botones y retorna el estado)
        ...
        mensaje = await update.message.reply_text(
            "📡 Por favor, selecciona el **Tipo de medio**:",
            reply_markup=InlineKeyboardMarkup(merge_keyboard_with_navigation(keyboard)),
            parse_mode='Markdown'
        )
        ...
        # ✅ CORRECCIÓN CLAVE: Debe ser ESTADO_TIPO_MEDIO para seguir el flujo Comunicacional.
        return ESTADO_TIPO_MEDIO
 #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< ESTADO_DESCRIPCION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

async def manejar_descripcion(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja la descripción y salta la pregunta de recursos para la alerta Verde operacional."""

    # 🔐 Activar modo modificación si aplica
    if context.user_data.get("pregunta_modificacion_id"):
        context.user_data["en_modo_modificacion"] = True
    ...
    # 🔑 Bifurcación CLAVE: Flujo de Modificación vs. Flujo Normal
    if context.user_data.get("en_modo_modificacion"):
        
        # 🗑️ CORRECCIÓN: Eliminar el mensaje actual del usuario
        if update.message:
            try:
                await update.message.delete()
            except Exception as e:
        ...
        # ➡️ Flujo NORMAL: (Tu código original)
...
async def manejar_descripcion_evento(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja la descripción del evento y avanza a solicitar las acciones tomadas."""
...
async def manejar_desc_verde_op(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Guarda la descripción del evento y pide las acciones tomadas (Verde Operacional)."""
    ...
    if context.user_data.get("en_modo_modificacion"):
        ...
        # ... (código para preguntar acciones tomadas)
        # ...
...
async def manejar_tipo_medio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja el tipo de medio y avanza al medio específico."""

    query = update.callback_query
    await query.answer()

    # 1. Captura y doble asignación (Tipo Evento/Medio)
    ...
    # 🔐 Flujo de Modificación: SI estamos en modificación, NO LLAMAMOS al post-manejador. 
    # Simplemente dejamos que el código pase a la bifurcación 3.3.
    if context.user_data.get("en_modo_modificacion") or context.user_data.get("pregunta_modificacion_id"):
        pass # ⚠️ Esto permite que el flujo continúe al siguiente paso (Medio Específico)
        
    # ➡️ Flujo Normal (Solo aquí guardamos historial)
    else:
        guardar_en_historial(context, ESTADO_TIPO_MEDIO)


    # 3.3. Bifurcación para la siguiente pregunta (Medio Específico) - LÓGICA DE AVANCE
    ...
    if tipo_medio_lower == "red social":
        ...
        mensaje = await query.message.reply_text(
            "📱 Por favor, selecciona la red social específica:",
        ...
        mensaje = await query.message.reply_text(
            f"📰 Por favor, ingresa el nombre del medio específico: (**{tipo_medio}**)",
            reply_markup=InlineKeyboardMarkup(crear_botones_navegacion()),
            parse_mode="Markdown"
        )
        context.user_data["pregunta_id"] = mensaje.message_id
...
# Esta función es el handler para ESTADO_MEDIO_ESPECIFICO
async def manejar_medio_especifico(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Maneja el nombre del medio específico. Si es modificación, finaliza el sub-flujo 
    y llama a manejar_post_modificacion.
    """
    
    # 1. Captura de dato (Maneja botón de Red Social o texto)
    if update.callback_query:
        query = update.callback_query
        await query.answer()
    ...
    if update.message:
        ...
        # 🔐 CRÍTICO: Si estamos en modo modificación, terminamos el proceso aquí.
    if context.user_data.get("en_modo_modificacion") or context.user_data.get("pregunta_modificacion_id"):
    ...
    # ➡️ Flujo Normal
        ...
        mensaje_pregunta = await mensaje_respuesta.reply_text(
            "📝 Por favor, ingrese el contenido difundido:",
            reply_markup=InlineKeyboardMarkup(crear_botones_navegacion())
        )
        context.user_data["pregunta_id"] = mensaje_pregunta.message_id
...
# 📋 Actualizar resumen y limpiar pregunta anterior
    await actualizar_resumen_estatico(update, context)
    await borrar_pregunta_anterior(context, update)
...
async def manejar_violencia(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if context.user_data.get("en_modo_modificacion"):
        ...
        # ☠️ Pregunta siguiente: amenaza a la vida
        keyboard = [[InlineKeyboardButton("✅ Sí", callback_data="si"), InlineKeyboardButton("❌ No", callback_data="no")]]
        reply_markup = InlineKeyboardMarkup(merge_keyboard_with_navigation(keyboard))

        await query.message.reply_text("¿Hubo amenaza a la vida?", reply_markup=reply_markup)
...
async def manejar_amenaza(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if context.user_data.get("en_modo_modificacion"):
        ...
        # 🔍 Pregunta siguiente: verificación
...
async def manejar_verificado(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if context.user_data.get("en_modo_modificacion"):
        ...
        # 📝 Pregunta siguiente: observaciones
...
async def manejar_observaciones(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 🔍 2. Capturar y guardar valor (Texto o Callback)
    observaciones = "Sin observaciones"
    if update.callback_query:
        query = update.callback_query
        await query.answer()
        # Se asume que el único callback relevante aquí es "No tengo observaciones"
        # Si tienes otros callbacks, deberás manejarlos aquí.
    ...
    # 📋 4. Actualizar resumen y limpiar pregunta anterior (Borra la pregunta del BOT)
    await actualizar_resumen_estatico(update, context)
    await borrar_pregunta_anterior(context, update)
    ...
    if context.user_data.get("en_modo_modificacion"):
        ...
        # 🗑️ CORRECCIÓN CRÍTICA: Eliminar el mensaje actual del usuario (si es texto)
        if update.message:
            try:
                await update.message.delete()
            except telegram.error.BadRequest as e:
                # Ignorar específicamente el error "Message to delete not found" (código 400 Bad Request)
                if "message to delete not found" not in str(e).lower():
                    print(f"Error al intentar borrar el mensaje de usuario: {e}")
            except Exception as e:
                print(f"Error al intentar borrar el mensaje de usuario: {e}")
                
        # ✅ Flujo de MODIFICACIÓN: Llama al post-manejador.
        return await manejar_post_modificacion(update, context)
        ...
        # 5b. 📎 Pregunta siguiente: multimedia
        botones = [[InlineKeyboardButton("➡️ Continuar", callback_data="continuar_multimedia")]]
        ...
        mensaje = await update.effective_chat.send_message(
            "📎 Por favor, adjunta hasta un máximo de 5 archivos de imágenes o videos. Si no tienes archivos, selecciona 'Continuar'.",
            reply_markup=reply_markup
        )
...
async def manejar_multimedia(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if mensaje.photo:
        ...
        await mensaje.reply_text("🚫 Por favor, envía solo imágenes, videos, documentos o audios.")
        return ESTADO_ESPERANDO_MULTIMEDIA
    ...
    if total_actual >= MAX_ARCHIVOS:
        mensaje_respuesta = (
            f"🎉 **Límite alcanzado.** Has adjuntado {MAX_ARCHIVOS} archivos.\n"
            "Presiona '➡️ Continuar' para guardar los cambios."
        )
    else:
        restantes = MAX_ARCHIVOS - total_actual
        plural = "archivo" if restantes == 1 else "archivos"
        mensaje_respuesta = (
            f"✅ **Archivo recibido** ({total_actual}/{MAX_ARCHIVOS}). "
            f"Puedes adjuntar **{restantes}** {plural} más, o presiona '➡️ Continuar' para guardar los cambios."
        )
    ...
    # 🔁 Flujo de modificación: NO cerrar aún, esperar botón
    if context.user_data.get("en_modo_modificacion"):
        return ESTADO_ESPERANDO_MULTIMEDIA
...
async def continuar_multimedia(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Finaliza la etapa multimedia y muestra el resumen final con opciones de envío o modificación."""
    ...
    if context.user_data.get("pregunta_modificacion_id") or context.user_data.get("en_modo_modificacion"):
        ...
        # 2. Asegurar que la lista exista (aunque esté vacía)
        context.user_data.setdefault("recursos_multimedia", [])

        # 3. Limpiar bandera de reinicio de modificación
        context.user_data.pop("modificacion_multimedia_iniciada", None)

        # 4. Actualizar resumen
        await actualizar_resumen_estatico(update, context)

        # 5. Fijar campo y redirigir al flujo de post-modificación
        context.user_data["campo_a_modificar"] = "recursos_multimedia"
        return await manejar_post_modificacion(update, context)
...
async def manejar_tipo_evento(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 🚨 NOTA: La lógica de 'verificar_modificacion' al inicio ya maneja los casos iniciales.
    # Esta función está más diseñada para los handlers de texto.
    ...
    # --- 1. Lógica cuando viene de BOTÓN (callback_query) ---
    if update.callback_query:
        query = update.callback_query
        await query.answer()
        ...
        # 🔑 CLAVE: Actualizar y luego decidir el flujo (Modificación vs. Normal)

        await actualizar_resumen_estatico(update, context)
        await borrar_pregunta_anterior(context, update) # Borrar la pregunta del Tipo de Evento

        # >>> CORRECCIÓN DE FLUJO CRÍTICA para botones <<<
        if context.user_data.get("en_modo_modificacion"):
        ...
        # Pregunta siguiente: Descripción
    ...
    # --- 2. Lógica cuando viene de TEXTO LIBRE (respuesta de 'otros') ---
    elif update.message:
        context.user_data["tipo_evento"] = update.message.text.strip()
        ...
        # >>> CORRECCIÓN DE FLUJO CRÍTICA para texto libre <<<
        if context.user_data.get("en_modo_modificacion"):
            context.user_data['campo_a_modificar'] = 'tipo_evento' # Fija el campo
            return await manejar_post_modificacion(update, context)
        # >>> Fin CORRECCIÓN DE FLUJO CRÍTICA <<<
...
async def manejar_recursos(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if context.user_data.get("en_modo_modificacion"):
        ...
        # 📝 Pregunta siguiente: acciones tomadas
        mensaje_pregunta = await update.message.reply_text(
            "📝 Por favor, ingresa las **acciones tomadas**:",
            reply_markup=InlineKeyboardMarkup(crear_botones_navegacion()),
            parse_mode='Markdown'
        )
        context.user_data["pregunta_id"] = mensaje_pregunta.message_id
...
async def manejar_acciones(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja las acciones tomadas y transita al estado de violencia o verificación según el nivel."""
...
async def manejar_acciones_tomadas(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja las acciones tomadas y muestra la siguiente pregunta de verificación."""
...
async def confirmar_y_enviar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Confirma el envío del reporte, muestra el resumen final y ofrece reiniciar."""
    ...
    try:
        ...
        # 🧾 Generar número de reporte y resumen
        ...
        await query.message.reply_text(
            ...
            f"<b>Número de Reporte:</b> {numero_reporte}\n\n"
            f"{resumen_final}",
            parse_mode="HTML"
        )

        # 🔁 Preguntar si desea iniciar otro reporte
        keyboard = [
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="¿Desea realizar otro reporte?",
            reply_markup=reply_markup
        )

    except Exception as e:
        import traceback
        logger.error(f"❌ Error al enviar el reporte: {e}\n{traceback.format_exc()}")
        await query.message.reply_text("❌ Ocurrió un error al enviar el reporte. Por favor, intenta de nuevo.")
...
async def cancelar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Cancela la conversación y limpia el contexto."""
    ...
    # 🧼 Limpieza completa del contexto
    context.user_data.clear()
...
async def manejar_verificacion_verde_op(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja la verificación del reporte de Alerta Verde Operacional y dirige a la etapa de multimedia/observaciones."""
    ...
    # 🧭 Guardar estado en historial si no está presente
    ...
    # 📦 Solicitud de observaciones/multimedia
    keyboard = [[InlineKeyboardButton("✅ Continuar sin multimedia", callback_data="continuar_sin_multimedia")]]
    reply_markup = InlineKeyboardMarkup(merge_keyboard_with_navigation(keyboard))

    await query.message.reply_text(
        "📝 **Observaciones / Multimedia Opcional:**\n\n"
        "Si desea añadir **observaciones** (texto), por favor escríbalas y envíelas ahora. Después podrá adjuntar fotos, videos o documentos.\n\n"
        "Cuando esté listo, pulse **'Continuar sin multimedia'** para ir al resumen, o simplemente envíe sus archivos.",
        reply_markup=reply_markup,
        parse_mode='Markdown'
    )
 ...
 #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< FECHA Y HORA >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Mantendré el nombre de la función que tienes para no romper tus referencias
from telegram import Update
from telegram.ext import ContextTypes
import json

async def manejar_datos_webapp(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Procesa los datos enviados desde la WebApp de selección de fecha y hora."""

    try:
        datos_raw = update.message.web_app_data.data
        print("📥 Datos recibidos:", datos_raw)  # 👈 Esto te mostrará los datos en consola
        ...
        if not fecha or not hora:
            await update.message.reply_text("⚠️ No se recibió una fecha y hora válidas. Intenta nuevamente.")
        ...
        await update.message.reply_text(
            f"✅ Fecha y hora recibidas:\n📅 {fecha}\n⏰ {hora}"
        )

        # Aquí puedes continuar al siguiente estado o mostrar resumen
        return ESTADO_ACTORES_CLAVE  # o el estado que corresponda en tu flujo

    except Exception as e:
        print("❌ Error al procesar datos de WebApp:", e)
        await update.message.reply_text("❌ Ocurrió un error al procesar la fecha y hora. Intenta nuevamente.")
        return ESTADO_FECHA_PUBLICACION
...
async def preguntar_fecha_webapp(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Envía el botón que abre la WebApp para seleccionar fecha y hora."""

    context.user_data["esperando_webapp_fecha"] = True

    boton_webapp = InlineKeyboardButton(
        text="📅 Seleccionar fecha y hora",
        web_app=WebAppInfo(url="https://selector-fecha-hora-bot-rat.vercel.app/")  # Reemplaza con tu URL real
    )

    reply_markup = InlineKeyboardMarkup([[boton_webapp]])
...
async def manejar_actores_clave(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja los actores clave y transita según tipo y nivel de reporte."""
    ...
    if update.message and update.message.text:
        ...
        # <--- APROXIMADAMENTE AQUÍ ESTABA LA LÍNEA 1659 ORIGINAL QUE FALLABA --->
        texto = update.message.text.strip() # Ahora es seguro usar .strip()
        
        # 1. Validación de entrada de texto (Si la tienes)
        if len(texto) < 3: 
            await update.message.reply_text("El campo de actores clave es muy corto. Por favor, ingrese una descripción más detallada.")
    ...
    if context.user_data.get("en_modo_modificacion"):
        ...
        if tipo == "operacional":
            if nivel == "verde":
                ...
                mensaje = await update.message.reply_text(
                    "📦 Por favor, ingresa los **recursos comprometidos**:",
                    parse_mode='Markdown',
                    reply_markup=InlineKeyboardMarkup(crear_botones_navegacion())
                )
                context.user_data["pregunta_id"] = mensaje.message_id
                ...
                mensaje = await update.message.reply_text("¿Hubo **violencia**?", parse_mode='Markdown', reply_markup=reply_markup)
                context.user_data["pregunta_id"] = mensaje.message_id
                # El estado se guarda antes. Solo se retorna el nuevo estado.
                return ESTADO_VIOLENCIA
            ...
            mensaje = await update.message.reply_text(
                "📰 Por favor, ingresa el **contenido difundido**:",
                parse_mode='Markdown',
                reply_markup=InlineKeyboardMarkup(crear_botones_navegacion())
            )
            context.user_data["pregunta_id"] = mensaje.message_id
        ...
        # ⚠️ Fallback si no se reconoce el flujo
        await update.message.reply_text("⚠️ No se pudo determinar el flujo. Intenta de nuevo.")
...
async def preguntar_fecha_manual(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 🧹 Limpiar pregunta anterior y actualizar resumen
    await borrar_pregunta_anterior(context, update)
    await actualizar_resumen_estatico(update, context)

    # 🗓️ Pregunta de fecha/hora
    mensaje = await update.effective_chat.send_message(
        "🗓️ *Por favor, escribe la fecha y hora del evento en este formato exacto:*\n\n"
        "`27/10/2025 11:00 AM`\n\n"
...
async def manejar_fecha_manual(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ *Formato inválido.* Por favor, usa este formato exacto:\n\n"
            "`27/10/2025 11:00 AM`\n\n"
            "Incluye el espacio entre fecha y hora, y usa AM o PM en mayúsculas.",
            parse_mode="Markdown"
        )
   ...
   # 📋 Actualizar resumen y limpiar pregunta anterior
    ...
    # 🔑 CLAVE: La condición para evitar guardar el estado temporal y redirigir
    if context.user_data.get("en_modo_modificacion"):
        # Flujo de Modificación: NO guarda en historial. 
        # Llama a la función que limpia la bandera y pide confirmación.
        return await manejar_post_modificacion(update, context)
    else:
    ...
    # 👥 Pregunta siguiente: actores clave
    mensaje = await update.message.reply_text(
